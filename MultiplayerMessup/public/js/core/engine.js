var GameEngine = (function (my) {
    ...
    my.UpdatePlayerPositions = function (playerPositionsDatas) {
        for (var id in playerPositionsDatas) {
            ...
            if (playerPositionData.positions.length == 2) {
                game.add.tween(player).to(
                    {
                        x: [playerPositionData.positions[0].x, playerPositionData.positions[1].x],
                        y: [playerPositionData.positions[0].y, playerPositionData.positions[1].y]
